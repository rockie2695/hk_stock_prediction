...
> **Warning:** 本系統僅供學習和研究使用，不構成任何投資建議。投資有風險，入市需謹慎。
...
- **MA均線疊加**: K線圖可疊加顯示 MA5 (短期)、MA10 (中期)、MA20 (長期) 移動平均線
- **股票對比圖**: 選擇兩支股票並排比較累計報酬率走勢和表現指標
- **技術指標展示**: 顯示 RSI、MACD、Stochastic、ADX、MFI、布林帶寬、ATR、量比等最新數值，作為信號依據
- **信號確認分析**: 每個 Buy/Sell 信號自動分析指標 alignment (支持/矛盾/中性)，幫助判斷信號可靠性
...
> **Warning:** 絕對不要將 `.env` 檔案提交至版本控制 (Git)。該檔案包含 Supabase 金鑰等敏感資訊。請確認 `.gitignore` 中已包含 `.env`。
...
儀表板包含兩個頁面 (側邊欄切換)：
- **📈 預測儀表板**: 分頁式設計，包含：
  - **📊 信號總覽**: 信號卡片 (1d/5d/20d) + 指標 alignment 分析 + 信號分佈
  - **📈 K線與指標**: K線圖 (含買賣信號) + 技術指標 + 股票對比
  - **🎯 信心度趨勢**: 信心度變化圖表 + Buy/Sell 閾值線
  - **📋 預測記錄**: 近期預測表格 + 匯出功能
  - **🔍 模型表現**: 滾動準確度 + 訓練指標 + 模型監控 + 市場狀態
- **💰 投資模擬器**: 自訂日期範圍、資金、時間範圍，模擬跟單收益
- **📊 策略回測**: 資金曲線、回撤、交易記錄、對比買入持有
- **💼 投資組合**: 持倉總覽、信號/信心度分佈、風險暴露
...
|---|---|
| `-d` | 背景運行 |
| `-p 8501:8501` | 映射 Streamlit 預設埠 |
| `--env-file .env` | 載入環境變數 |
| `--name stock-dashboard` | 容器名稱 |

> **Warning:** 絕對不要在 Docker 鏡像中硬編碼 Supabase 金鑰或任何 `.env` 內容。使用 `--env-file` 在運行時注入環境變數。切勿將 `.env` 檔案 COPY 到鏡像內部。

> **Note:** 使用 GPU 加速 (CatBoost) 時，Docker 需要額外安裝 NVIDIA Container Toolkit。確保宿主機已安裝 NVIDIA 驅動程式，並使用 `--gpus all` 參數運行容器。
...
│       ├── 2_backtest.py      # 策略回測頁面
│       └── 3_portfolio.py     # 投資組合頁面
...
└── migrations/
    └── 003_extended_features.sql # 擴展特徵說明 (無需資料庫變更)
```

> **Warning:** `.env` 和 `.pkl` 檔案不應提交至版本控制。`.gitignore` 已配置忽略這些檔案。如果意外提交，請立即從 Git 歷史中清除。
...
> **Warning:** SMOTE 僅在 TimeSeriesSplit 的訓練折 (training fold) 上套用，絕對不能跨越驗證折。這是防止數據洩漏的關鍵措施。如果 SMOTE 在整個訓練集上套用，模型會在驗證集上獲得虛高的 AUC。

> **Note:** GPU 加速需要 NVIDIA GPU 及已安裝的 NVIDIA 驅動程式。CatBoost GPU 訓練會使用更多記憶體，建議至少 4GB VRAM。非 NVIDIA GPU (如 AMD) 不支援。
...
> **Note:** 股票預測本身非常困難。即使是大型對沖基金，AUC 也通常在 0.55-0.65 之間。AUC 0.55-0.60 代表模型有一定預測能力，但並非「錯誤」。

> **Warning:** 過去表現不代表未來結果。模型在歷史數據上的 F1/AUC 不保證未來收益。切勿將模型表現等同於交易保證。使用投資模擬器時，務必考慮交易成本和滑點的影響。
...
> **Note:** 交易成本按香港標準計算：佣金 0.1% (最低 HKD 20) + 印花稅 0.13% (僅賣出)。此費率可能因券商而異，實際成本請以你的券商為準。不同市場的交易成本差異很大，請勿直接套用於其他市場。
...
### 模型漂移檢測
- 比較最近 14 天 vs 60 天的預測準確度 (使用真實價格驗證)
- 準確度下降 >10% = 高度警報
- 準確度下降 >5% = 中度警報
...
> **Warning:** 特徵工程嚴禁 Look-ahead Bias。這是量化交易中最常見的錯誤之一。所有特徵必須僅使用當天及之前的数据計算，確保模型在實際交易中能複製相同的表現。

> **Warning:** `.env` 檔案包含 Supabase 金鑰和資料庫憑證。絕對不要提交至 Git，也不要出現在 Docker 鏡像中。建議在 `.gitignore` 中明確排除 `.env`、`*.pkl` 和 `models/` 目錄。
...
> **Note:** 股票預測不是有明確答案的問題。市場充滿噪音和隨機性，AUC 0.55-0.60 已代表具有統計顯著性的預測能力。
...
> **Warning:** SMOTE 僅在訓練折上套用，絕對不能在驗證折或測試集上套用。
...
### Q: 信號確認分析 (Signal Confirmation) 是什麼？
A: 每個 Buy/Sell 信號下方會自動分析當前指標是否支持該信號。例如：
- ✅ RSI 42 偏低 → 支持 Buy
- ❌ MACD -1.06 空頭動能 → 矛盾 Buy
- ➖ ADX 9 盤整 → 信號可靠性降低
最後顯示「幾項支持、幾項中性、幾項矛盾」，幫助快速判斷信號可靠性。Hold 信號不會顯示分析。

### Q: 技術指標是即時計算的嗎？
A: 是的。指標從 yfinance 即時獲取 2 年 OHLCV 數據，透過 `compute_features()` 計算。結果緩存 5 分鐘 (TTL=300s)，避免重複計算。如果今日市場尚未收盤，今日數據的 Close 可能為 NaN，系統會自動跳過該筆數據。
...
**第一階段 (10 Features)：**
- **情緒分析 (3)**: 從新聞/社群獲取市場情緒分數
- **板塊輪動 (3)**: 追蹤行業板塊ETF動量，判斷資金輪動方向
- **沽空比率 (3)**: 監控沽空活動變化，高沽空可能暗示看跌情緒
- **互聯互通 (1)**: 南向資金流向反映內地投資者情緒

**第二階段 (5 Features)：**
- **市場狀態 (3)**: 使用恒指 MA50/MA200 交叉判斷牛市/熊市/震盪

所有擴展特徵在無法獲取數據時自動回退為預設值，不影響核心功能。

### Q: 情緒分析 (Sentiment) 如何運作？
A: 情緒分析從東方財富 (AKShare) 獲取個股新聞，使用關鍵字匹配計算情緒分數。正面關鍵字 (如「上漲」、「突破」、「利好」) 加分，負面關鍵字 (如「下跌」、「跌破」、「利空」) 扣分。分數範圍 -1 (極度看空) 到 +1 (極度看多)。

**特徵說明：**
| 特徵 | 說明 | 用途 |
|---|---|---|
| `sentiment_5d` | 5天滾動情緒平均 | 短期市場情緒 |
| `sentiment_10d` | 10天滾動情緒平均 | 中期市場情緒 |
| `sentiment_change` | 情緒變化 (5d - 10d) | 情緒動量，正數表示情緒改善 |
...
### Q: 板塊輪動 (Sector) 如何運作？
A: 板塊輪動追蹤 6 個恒生行業 ETF (科技、金融、地產、能源、醫療、消費) 的動量，判斷資金輪動方向。當某板塊動量強於恒指，表示資金流入該板塊。
...
| 特徵 | 說明 | 用途 |
|---|---|---|
| `sector_momentum_5d` | 5天板塊ETF動量 | 短期板塊輪動 |
| `sector_momentum_20d` | 20天板塊ETF動量 | 中期板塊輪動 |
| `sector_vs_hsi` | 板塊相對恒指表現 | 板塊超額報酬 |

### Q: 沽空比率 (Short Selling) 如何運作？
A: 沽空比率追蹤市場沽空活動變化。高沽空可能暗示看跌情緒，但極端沽空可能觸發軋空 (short squeeze)。使用 yfinance 恒指數據和成交量模式作為代理指標。

**特徵說明：**
| 特徵 | 說明 | 用途 |
|---|---|---|
| `short_sell_ratio` | 即時沽空比率 | 當前市場看空程度 |
| `short_sell_ratio_5d` | 5天滾動沽空比率 | 短期沽空趨勢 |
| `short_sell_ratio_change` | 沽空比率變化 | 沽空動量，正數表示沽空增加 |

**注意事項：**
- 港股沽空數據有延遲 (T+1)，使用成交量模式作為代理
- 極端沽空 (>0.8) 可能觸發軋空，反而是利多信號

### Q: 互聯互通 (Connect Flow) 如何運作？
A: 互聯互通追蹤南向資金 (內地投資者買港股) 的流向。南向資金流入增加時，表示內地投資者看好港股。使用恒指成交量和價格模式作為代理指標。

**特徵說明：**
| 特徵 | 說明 | 用途 |
|---|---|---|
| `southbound_net_5d` | 5天淨南向資金流 | 短期資金流向 |
| `southbound_momentum` | 南向資金動量 | 資金流入加速度 |
| `connect_sentiment` | 互聯互通市場情緒 | 內地投資者情緒 |

**情緒分數計算邏輯：**
| 條件 | 分數 | 說明 |
|---|---|---|
| HSI 5日報酬 > 1% | +0.3 | 恒指溫和上漲 |
| HSI 5日報酬 > 3% | +0.5 | 恒指強勢上漲 |
| HSI 5日報酬 < -1% | -0.3 | 恒指溫和下跌 |
| HSI 5日報酬 < -3% | -0.5 | 恒指強勢下跌 |
| 上漲 + 成交量 > 1.5倍均量 | +0.6 | 放量上漲，強勢信號 |
| 下跌 + 成交量 > 1.5倍均量 | -0.6 | 放量下跌，弱勢信號 |

### Q: 增量學習 (Online Learning) 如何運作？
A: 增量學習允許模型在不完全重訓的情況下，用近期數據進行增量更新。當上次全量訓練超過 7 天時，系統會自動使用最近 60 天數據更新模型權重，節省時間和計算資源。

**運作機制：**
1. **觸發條件**: 上次全量訓練 > 7 天前
2. **訓練數據**: 最近 60 天歷史數據
3. **更新方式**: 使用 warm-start (熱啟動) 載入舊模型權重，繼續訓練
4. **模型保存**: 更新後的模型覆蓋當前模型

**注意事項：**
- 目前預設關閉 (USE_ONLINE_LEARNING=False)
- 增量學習可能導致模型漂移，建議定期全量重訓
- 僅更新 XGBoost 和 LightGBM 模型

### Q: 動態權重 (Dynamic Weighting) 如何運作？
A: 動態權重追蹤每個模型 (XGBoost/LightGBM/RandomForest/CatBoost) 的近期表現，使用 EMA (指數移動平均) 調整集成權重。表現較好的模型獲得較高權重，使集成預測更準確。

**運作機制：**
1. **權重初始化**: 新股票/時間範圍使用等權重 (各 25%)
2. **EMA 更新**: `新權重 = α × 近期表現 + (1-α) × 舊權重`
3. **權重正規化**: 確保所有權重總和為 1
4. **最低權重**: 每個模型最低權重 10%，避免完全忽略
...
| 特徵 | 模組 | 說明 |
|---|---|---|
| `sentiment_5d` | `src/sentiment.py` | 5天滾動新聞情緒分數 (東方財富) |
| `sentiment_10d` | `src/sentiment.py` | 10天滾動新聞情緒分數 |
| `sentiment_change` | `src/sentiment.py` | 情緒變化 (5天 vs 10天) |
| `sector_momentum_5d` | `src/sector.py` | 5天板塊ETF動量 (3033.HK, 3086.HK, 3046.HK, 3069.HK, 3053.HK, 3097.HK) |
| `sector_momentum_20d` | `src/sector.py` | 20天板塊ETF動量 |
| `sector_vs_hsi` | `src/sector.py` | 板塊相對恒指表現 |
| `short_sell_ratio` | `src/short_selling.py` | 估算沽空比率 (基於價格/成交量模式) |
| `short_sell_ratio_5d` | `src/short_selling.py` | 5天滾動沽空比率 |
| `short_sell_ratio_change` | `src/short_selling.py` | 沽空比率變化 |
| `southbound_net_5d` | `src/connect_flow.py` | 5天淨南向資金流 |
| `southbound_momentum` | `src/connect_flow.py` | 南向資金動量 |
| `connect_sentiment` | `src/connect_flow.py` | 互聯互通市場情緒 |

#### 第二階段：模型特徵 — 3 項新特徵

| 特徵 | 模組 | 說明 |
|---|---|---|
| `market_regime` | `src/regime.py` | 市場狀態：0=熊市, 1=震盪, 2=牛市 |
| `regime_confidence` | `src/regime.py` | 狀態分類信心度 |
| `hsi_trend_50_200` | `src/regime.py` | 恒指50日均線 vs 200日均線比率 |

#### 第三階段：儀表板 — 2 個新頁面

| 頁面 | 檔案 | 說明 |
|---|---|---|
| 回測 | `app/pages/2_backtest.py` | 資金曲線、回撤、交易記錄、對比買入持有 |
| 投資組合 | `app/pages/3_portfolio.py` | 持倉總覽、信號/信心度分佈、風險暴露 |
...
```env
# 第一階段
USE_SENTIMENT=True     # 新聞情緒特徵
USE_SECTOR=True        # 板塊輪動特徵
USE_SHORT_SELL=True    # 沽空比率特徵
USE_CONNECT=True       # 南向資金流特徵
...
| 改進 | 說明 |
|---|---|
| **分頁式儀表板** | 預測儀表板重構為 5 個分頁：信號總覽、K線與指標、信心度趨勢、預測記錄、模型表現 |
...
| **信號確認分析** | 每個 Buy/Sell 信號下方自動分析指標 alignment，顯示「幾項支持/矛盾/中性」及每個指標的具體判斷 |
| **滾動準確度追蹤** | 新增30天滾動窗口計算真實預測準確度，驗證 Buy 後價格是否上漲、Sell 後價格是否下跌 |
| **訓練指標展示** | 新增訓練指標面板，顯示各股票各時間範圍的 F1 Score、AUC Score、冠軍模型類型 |
| **Bug Fix: MFI 缺失** | 修正 `get_latest_indicators()` 回傳字典缺少 `mfi` 鍵，導致 MFI 永遠顯示 `—` |
| **Bug Fix: MA50 偏離** | 修正 MA50 偏離顯示錯誤 (原始值 -0.05% → 正確值 -4.85%)，因小數未乘以 100 |
| **Bug Fix: 準確度顯示** | 修正滾動準確度顯示 5000.0% (因 `:.1%` 格式誤將百分比當小數處理) |
| **Bug Fix: 鍵名錯誤** | 修正 `total_predictions`/`correct_predictions` → `total`/`correct` (與 `calculate_accuracy` 回傳值一致) |
| **NaN 資料處理** | 修正 yfinance 今日數據 Close 為 NaN 導致所有指標顯示 `—`，改為先 `dropna(subset=["Close"])` |
...
| Bug | 模組 | 根本原因 | 修正 |
|---|---|---|---|
| **情緒: 0 articles** | `src/sentiment.py` | `str(int("0700"))` → `"700"`, AKShare 需要 5 位格式 | 使用 `.zfill(5)` → `"00700"` |
| **情緒: all scores = 0** | `src/sentiment.py` | 日期欄位 `'发布时间'` 未偵測 → 選擇 `'關鍵詞'` | 新增 `'时间'`/`'時間'` 到日期欄位檢查 |
| **情緒: all scores = 0** | `src/sentiment.py` | 文字欄位首先匹配標題，太短無法匹配關鍵字 | 優先使用內容 (`'新闻内容'`) 而非標題 |
| **沽空: timedelta error** | `src/short_selling.py` | `fetch_stock_short_selling(code, days=30)` — 函數期望 `years` | 改為 `years=1` |
| **板塊 ETF: delisted** | `src/sector.py` | `3022.HK` (金融) 在 Yahoo Finance 已下架 | 改為 `3086.HK` (恒生中國銀行) |
| **板塊 ETF: delisted** | `src/sector.py` | `3048.HK` (地產) 在 Yahoo Finance 已下架 | 改為 `3097.HK` (恒生物業) |

### 修改的檔案
- `app/streamlit_app.py` — 分頁式儀表板重構；新增 K線圖、技術指標、信號確認分析、滾動準確度、訓練指標展示；修正 MFI、MA50、準確度顯示 bug
...
> **Warning:** 過去表現不代表未來結果。本系統的所有回測和模擬結果僅供參考，不應作為實際投資決策的依據。
